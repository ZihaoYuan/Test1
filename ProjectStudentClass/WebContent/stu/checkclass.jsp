...
String sql="select count(*) from "+u;
...
while(rs.next()){
	...
	<td><form action="deleteclass.jsp?id=<%=rs.getInt(1) %>&currentPage=<%=currentPage%>" method="post">
...
<a href="checkclass.jsp?currentPage=1">首页</a>&nbsp;&nbsp;&nbsp;&nbsp;
<a href="checkclass.jsp?currentPage=<%=currentPage-1 %>">上一页</a>&nbsp;&nbsp;&nbsp;&nbsp;
<%
for(int i=currentPage-5;i<currentPage;i++){
	if(i>0){
		
	%>
	<a href="checkclass.jsp?currentPage=<%=i %>"><%=i %></a>
	<%
	}
}
...
%>&nbsp;&nbsp;&nbsp;&nbsp;
<a href="checkclass.jsp?currentPage=<%=currentPage+1 %>">下一页</a>&nbsp;&nbsp;&nbsp;&nbsp;
<a href="checkclass.jsp?currentPage=<%=totalPage %>">末页</a></center>
</td></tr>
<tr><td colspan="4"><center>
第<%=currentPage %>页
</center>
